    ...
    <div className="min-h-screen bg-gray-light">
      ...
      <div className="bg-gradient-to-r from-primary to-primary-light text-white pt-32 pb-16">
        <div className="container mx-auto px-6">
          ...
          <h1 className="text-4xl font-bold font-heading">Directorio Médico e Institucional</h1>
      ...
      <div className="container mx-auto px-6 py-10">
        ...
        <div className="bg-white rounded-card shadow-card p-5 mb-10 flex flex-col sm:flex-row gap-4 max-w-4xl mx-auto">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Buscar por nombre..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" />
          </div>
          <select value={filtroEsp} onChange={e => setFiltroEsp(e.target.value)}
            className="border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-dark focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors sm:w-72">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {[...Array(6)].map((_, i) => <div key={i} className="h-24 bg-gray-200 rounded-card animate-pulse" />)}
          </div>
        ) : (
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {currentItems.map((m) => (
                <div key={m.id} className="bg-white rounded-xl shadow-sm border border-gray-100 hover:border-primary/50 hover:shadow-md transition-all p-5 flex items-center gap-4">
