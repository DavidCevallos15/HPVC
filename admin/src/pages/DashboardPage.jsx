...
export default function DashboardPage() {
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold font-heading text-neutral-900">Dashboard</h1>
        <p className="text-neutral-500 text-sm mt-1">Bienvenido de vuelta, <strong>{user?.nombre}</strong>.</p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          {cards.map(({ label, value, total, Icon, color, link }) => (
            <Link key={label} to={link}
              className="bg-white rounded-xl shadow-sm border border-neutral-100 p-5 hover:shadow-md transition-all group">
              <div className="flex items-center justify-between mb-4">
                <div className={`size-10 rounded-xl ${color} flex items-center justify-center`}>
                  <Icon size={20} className="text-white" />
                </div>
                {total != null && (
                  <span className="text-xs text-neutral-400">{value}/{total}</span>
                )}
              </div>
              <div className="text-3xl font-semibold text-neutral-900">{value}</div>
              <div className="text-sm text-neutral-500 mt-1 group-hover:text-primary transition-colors">{label}</div>
            </Link>
          ))}
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-100 p-6">
        <h2 className="font-semibold font-heading text-neutral-900 mb-4">Acciones rápidas</h2>
