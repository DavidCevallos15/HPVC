...
var Combo = /** @class */ (function (_super) {
    ...
    Combo.prototype.setConfig = function (config) {
        ...
        setTimeout(function () {
            ...
            _this.events.on(types_1.BaseElementEvent.change, function (change) {
                if (change !== "load") {
                    _this.validate();
                }
            });
...
function initHotkeys(grid) {
    ...
    focusableKeyManager.addHotKey("enter", function () {
        if (cellSelecting(grid.config.selection) && grid.config.editing) {
